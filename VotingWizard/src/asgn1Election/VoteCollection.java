 ...
 * 
 * <p>
 * As before, please note the name clash between
 * <code>asgn1Election.Collection</code> and <code>java.util.Collection</code>.
 * 
 * @author hogan
...
public class VoteCollection implements Collection {
	...
	private CandidateIndex getPrefthKey(Vote v, TreeMap<CandidateIndex, Candidate> cds, int pref) {
		...
		int preventInfiniteLoop, failAtZero;

		preventInfiniteLoop = numCandidates;
		failAtZero = 0;

		do {
			//Search for the next preference, return once found.
			...
			preventInfiniteLoop--;
		} while (testCandidateForNull == null && preventInfiniteLoop > failAtZero);

		return findCandidate;
	}

	/**
	 * Creates array containing a 1 if the candidate is still active or 0 if the
	 * candidate has been eliminated.
	 * 
	 * Used to help prevent duplicate vote counting
	 * 
	 * @param cds <code>TreeMap</code> Candidate list 
	...
	/**
	 * Checks that a vote is actually a vote that needs to be counted. Method is
	 * a fail safe to ensure votes are not counted multiple times.
	 * 
	 * If the preference is less than the current vote being eliminated and the
	 * particular preference has an active candidate - return false.
	 * 
	 * @param activeCandidates <code>int[]</code> 1 for active 0 for eliminated
	 * @param vote <code>Vote</code> List of unordered votes
	 * @param compareVotePref <code>int</code> The single preference vote to compare.
	...
	private boolean checkForFalsePositive(int[] activeCandidates, Vote vote, int compareVotePref) {
		...
		for (int votePref : vote) {
			if (votePref < compareVotePref && activeCandidates[counter] == candidateIsActive) {
				//Return false as soon as false positive found
				return false; 
			}
			counter++;
		}

		//Return true is vote safe to count
		return true; 
	}
	 ...
	 * 
	 * @param vote <code>Vote</code> list to invert and index
	 * @param candidateToEliminate <code>int</code> Current candidate preference that is being eliminated
	 * @return Returns 0 based index of the candidate to eliminate, returns -1
	 *         if no index found
	 */
	 ...
	 * 
	 * Only return true if the index and the candidate to eliminate 
	 * are true AND to prevent false positives, check if the vote 
	 * has already been counted.
	 * 
	 * @param cds <code>TreeMap</code> Containing the candidates still active in this
	 *            election.
	 * @param elim <code>CandidateIndex</code> being eliminated
	 * @param vote <code>Vote</code> being processed
	 * @param eliminateIndex <code>int</code> Index of the eliminated preference
	 * @return True if vote valid for preference count, False if no preference
